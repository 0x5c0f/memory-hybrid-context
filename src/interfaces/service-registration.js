  ...
  api.registerService({
    ...
    start: () => {
      ...
      if (cfg.enabled && cfg.indexing.async !== false) {
        const runIndexer = () => {
          try {
            const result = runtime.processIndexJobs({ limit: cfg.indexing.batchSize });
            if (result.completed > 0 || result.failed > 0) {
              api.logger.info(
                `memory-hybrid-context: indexer claimed=${result.claimed} completed=${result.completed} failed=${result.failed} skipped=${result.skipped}`,
              );
            }
          } catch (err) {
      ...
      if (cfg.enabled && cfg.ttl.autoCleanup !== false) {
        const runCleanup = () => {
          try {
            const result = runtime.cleanupExpiredRecords({ limit: Math.max(25, cfg.indexing.batchSize * 4) });
            if (result.cleaned > 0 || result.jobsSkipped > 0) {
              api.logger.info(
                `memory-hybrid-context: cleanup matched=${result.matched} cleaned=${result.cleaned} jobsSkipped=${result.jobsSkipped}`,
              );
            }
            if (cfg.ttl.purgeAfterDays > 0) {
              const purge = runtime.purgeExpiredRecords({ limit: Math.max(25, cfg.indexing.batchSize * 4) });
              if (purge.purged > 0 || purge.deletedArchives > 0) {
                api.logger.info(
                  `memory-hybrid-context: purge matched=${purge.matched} purged=${purge.purged} deletedArchives=${purge.deletedArchives}`,
                );
              }
            }
          } catch (err) {
      ...
      if (cfg.enabled && Math.max(0, Math.floor(Number(cfg.commit.idleMinutes) || 0)) > 0) {
        ...
        const runIdleCommit = () => {
          try {
            const result = runtime.commitIdleSessions({ limit: Math.max(10, cfg.capture.maxCandidatesPerTurn * 4) });
            if (result.committedRecords > 0 || result.matchedSessions > 0) {
              api.logger.info(
                `memory-hybrid-context: idle-commit matchedSessions=${result.matchedSessions} committedSessions=${result.committedSessions} committedRecords=${result.committedRecords}`,
              );
            }
          } catch (err) {
