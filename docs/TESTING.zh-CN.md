...
### 3.4 多 agent 强隔离测试（`isolation.mode = agent`）

这个测试用于验证 `agent_id` 隔离是否生效。
...
1. `agent-a` 能检索到 `这是 A 侧记录`
2. `agent-b` 默认检索不到 `这是 A 侧记录`
...
openclaw mhm commit "当前联调项目使用 memory-hybrid-context 作为主记忆插件。" --type decision --session frontend-e2e-seed-1 --l0 "主记忆插件" --l1 "当前联调项目使用 memory-hybrid-context 作为主记忆插件。" --l2 "当前联调项目使用 memory-hybrid-context 作为主记忆插件，当前正式测试版本为 0.2.0。"
...
openclaw mhm commit "2026-03-03 已完成 memory-hybrid-context 0.2.0 正式包联调准备。" --type event --session frontend-e2e-seed-1 --l0 "联调完成" --l1 "2026-03-03 已完成 memory-hybrid-context 0.2.0 正式包联调准备。" --l2 "2026-03-03 已完成 memory-hybrid-context 0.2.0 正式包联调准备，可在前端通过自然语言触发记忆召回。"
```
...
1. 回答中包含 `memory-hybrid-context 0.2.0 正式包联调准备`
...
### 4.5 系统命令不触发召回测试（`/new` / `/reset`）

这个测试用于确认：

1. 系统命令启动提示不会被注入 recall 包
2. 召回只在真实用户提问场景触发

测试步骤：

1. 在前端执行 `/new`（或 `/reset`），只观察启动提示，不追加业务问题
2. 检查启动提示内容
3. 再发送一条真实提问（例如：`你还记得当前联调标签是什么吗？`）

通过标准：

1. 启动提示中不应出现 `<memory-hybrid-context>` 块
2. 随后的真实提问仍应正常触发召回并命中已写入记忆
...
openclaw mhm --agent-id agent-a purge --session iso-a
openclaw mhm --agent-id agent-b purge --session iso-b
